...
- **pipeline/runner.py** — render video qua browser-automation Flow/Veo. Request generation có reCAPTCHA phải đi qua capability `flow_api_proxy` của Chrome extension để token + request cùng browser/IP; VPS chỉ poll/download. `run_scene_job` (cảnh của project) & `run_video_job` (job lẻ) → `_generate_one`. `_resolve_variant` đổi `_t2v_`→`_r2v_`/`_i2v_`; model Veo portrait KHÔNG-lite cần chèn infix `_s_` (vd `veo_3_1_r2v_s_fast_portrait_ultra`). `_try_auto_merge` tự ghép các cảnh thành `merged_file` khi tất cả xong. Voiceover của project dùng native audio Flow/Veo; `_tts_pcm` qua Gemini chỉ còn phục vụ công cụ Tạo Audio độc lập.
