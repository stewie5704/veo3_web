...
class SceneScript(BaseModel):
    ...
    chars: list[str] = []
    shot: str = ""
    lens: str = ""              # focal length + aperture (vd "50mm f/1.4") — số cụ thể để Veo bám ống kính thật
    camera_move: str = ""
    lighting: str = ""
    mood: str = ""
    audio: str = ""              # sound design: ambient + 1 sfx theo hành động + music mood (KHÔNG lời thoại)
    depth: str = ""              # 3-plane composition: FG/MG/BG để ra chiều sâu điện ảnh
    atmos: str = ""              # physics/atmospheric anchor: 1 chi tiết vật lý (gió/khói/hạt bụi/mồ hôi) chống cảnh "chết đứng"
    continuity: str = ""         # hint nối cảnh trước (khi chain mode) — vd "matches previous scene lighting direction and camera height"
...
def _build_shot_prompt(present: list, scene: SceneScript, style_lock: str) -> str:
    ...
    if body:
        parts.append(body)
    if (scene.depth or "").strip():
        parts.append(f"Depth: {scene.depth.strip()}.")
    if (scene.atmos or "").strip():
        parts.append(f"Atmosphere: {scene.atmos.strip()}.")
    if (scene.continuity or "").strip():
        parts.append(f"Continuity: {scene.continuity.strip()}.")
    if style_lock.strip():
...
characters[]: hồ sơ TOÀN BỘ các nhân vật xuất hiện trong kịch bản (kể cả nhân vật phụ, đám đông, động vật - KHÔNG id, theo thứ tự xuất hiện), các trường TÁCH RỜI tiếng Anh: name, role, age, gender_presentation, face, eyes, hair, skin_tone (TRUNG TÍNH — không nhãn chủng tộc), build, wardrobe_top, wardrobe_bottom, footwear, headwear, accessories, distinguishing_marks (BẮT BUỘC), anchor (2 CHI TIẾT PHÂN BIỆT ghép bằng " + ": MỘT vật/phụ kiện độc + MỘT nét mặt/thân thể độc — vd "silver locket + scar above left eyebrow" — kết hợp 2 lớp lock chặt hơn, dẫn đầu nhận dạng), palette, voice, tts_voice (Kore/Aoede/Leda=NỮ, Puck/Charon/Orus=NAM, khớp giới).
...
Mỗi cảnh: beat ({lang_label} - BẤT KỂ GỐC LÀ GÌ), chars (list KHÓA "CHAR_n" — CHỈ khóa đã có), image ({lang_label} - BẮT BUỘC), action ({lang_label} - BẮT BUỘC — LIỆT KÊ 2-4 CỬ CHỈ/MICRO-MOTION quan sát được, vd "nghiêng đầu, ngón tay xoa gờ tách, liếc mắt xuống rồi ngước lên"; CẤM tính từ cảm xúc trần trụi không kèm động tác — mọi cảm xúc PHẢI hiển thị qua CỬ CHỈ CƠ THỂ), shot/lens/camera_move/lighting/mood (English, ĐA DẠNG cú máy; LENS BẮT BUỘC có focal length + aperture số cụ thể "50mm f/1.4"; ánh sáng nêu NGUỒN + nhiệt màu + THỜI ĐIỂM SỐ), depth (English — bố cục 3 lớp "FG: ...; MG: ...; BG: ..."), atmos (English — 1 chi tiết vật lý/khí quyển: gió/khói/hạt bụi/mồ hôi/nến), continuity (English, "" ở cảnh đầu chunk — hint nối cảnh trước "matches previous lighting direction and camera height"), audio (ambient + 1 sfx theo hành động + music mood "low and unobtrusive"; KHÔNG lời thoại), speaker (KHÓA hoặc ""), dialogue ({lang_label}{keep}), prompt (MỘT đoạn TIẾNG ANH cho Veo: [shot+lens+camera]->[hành động]->[bối cảnh+thời điểm]->[ánh sáng có nguồn]->[mood+film-stock/grade]; gọi nhân vật bằng TÊN, KHÔNG dùng "CHAR_n", KHÔNG tả lại ngoại hình — hệ thống tự chèn; KHÔNG viết lời thoại/says/voiceover/sings — Veo câm lời).
CHỈ JSON hợp lệ, KHÔNG markdown.
...
- "action": hành động cụ thể ({lang_label}) — LIỆT KÊ 2-4 CỬ CHỈ/MICRO-MOTION quan sát được (vd "khẽ nghiêng đầu về bên trái, ngón cái xoa gờ tách trà, liếc mắt xuống rồi ngước chậm lên, hít thở sâu một hơi"). CẤM tính từ cảm xúc trần trụi ("buồn", "vui", "lo lắng") mà không có động tác kèm — MỌI cảm xúc PHẢI hiển thị qua CỬ CHỈ CƠ THỂ (mắt, tay, vai, môi, hơi thở, khớp). Diễn viên KHÔNG đứng bất động.
- "shot","lens","camera_move","lighting","mood": thông số quay (English) — vd "medium close-up","50mm f/1.4","slow push-in","soft window key + rim backlight","tense". LENS BẮT BUỘC có focal length + aperture số cụ thể (vd "50mm f/1.4", "24mm f/8", "85mm f/2.0") — Veo có training data ống kính thật. ÁNH SÁNG phải nêu NGUỒN VẬT LÝ + nhiệt màu + THỜI ĐIỂM SỐ (vd "soft window key from camera-left, warm 3200K, 4:30 PM golden hour"), KHÔNG nói chung chung "cinematic lighting". PHẢI ĐA DẠNG cú máy giữa các cảnh.
- "depth": bố cục 3 lớp CHIỀU SÂU (English) — vd "FG: blurred fern leaf; MG: subject sharp at counter; BG: bokeh warm shop lights". LUÔN có 3 lớp FG/MG/BG để ra chiều sâu điện ảnh, KHÔNG được để 1 lớp phẳng.
- "atmos": 1 CHI TIẾT VẬT LÝ/KHÍ QUYỂN (English) — vd "wind lifts fabric edge left", "steam curls from cup", "dust motes drift in sunbeam", "single sweat bead on temple", "candle flame flickers". BẮT BUỘC có để cảnh KHÔNG chết đứng.
- "continuity": hint nối cảnh trước (English, để "" ở cảnh 1) — vd "matches previous scene's lighting direction and camera height", "carries over warm rim from prior shot", "continues from last frame's subject position camera-right". Áp cho chain-mode chuyển cảnh mượt.
...
(1) characters[] — HỒ SƠ TOÀN BỘ CÁC NHÂN VẬT XUẤT HIỆN TRONG KỊCH BẢN (kể cả nhân vật phụ, đám đông, động vật nếu có tham gia hành động) khoá để cùng một người trông GIỐNG HỆT ở mọi cảnh (KHÔNG ảnh tham chiếu). QUY TẮC TÊN: cast = ĐÚNG nhân vật có tên trong kịch bản; GIỮ NGUYÊN tên y như người dùng (đưa vào "name"); KHÔNG đổi/dịch tên; KHÔNG bịa nhân vật. Kịch bản đã tả ngoại hình thì BÁM SÁT; phần thiếu mới suy luận hợp lý và CỐ ĐỊNH. Các TRƯỜNG TÁCH RỜI (English): name, role, age, gender_presentation, face, eyes, hair, skin_tone (TRUNG TÍNH — không nhãn chủng tộc), build ("height=…cm; build=…"), wardrobe_top, wardrobe_bottom, footwear, headwear, accessories, distinguishing_marks (BẮT BUỘC), anchor (2 CHI TIẾT PHÂN BIỆT ghép bằng " + ": MỘT vật/phụ kiện độc + MỘT nét mặt/thân thể độc — vd "jade bracelet + laugh lines with mole on right cheek". Kết hợp 2 lớp = ID lock chặt hơn, DẪN ĐẦU nhận dạng mọi cảnh), palette, voice, tts_voice (giọng đọc — Kore/Aoede/Leda cho NỮ, Puck/Charon/Orus cho NAM, KHỚP giới tính; nhân vật khác nhau giọng khác nhau). MỖI nhân vật một bộ trang phục cố định. KHÔNG gán id; liệt kê theo thứ tự XUẤT HIỆN.
...
- "shot","lens","camera_move","lighting","mood" (English; BÁM SÁT mô tả của user; KHÔNG tự ý thêm thắt các cú máy phức tạp hay ánh sáng nếu user không yêu cầu, giữ mức cơ bản/trung lập). LENS BẮT BUỘC có focal length + aperture số cụ thể (vd "50mm f/1.4", "35mm f/2.8"). ÁNH SÁNG nêu NGUỒN + nhiệt màu + THỜI ĐIỂM SỐ khi suy được (vd "warm 3200K, 5 PM golden hour").
- "depth": bố cục 3 lớp FG/MG/BG (English) — vd "FG: blurred cup edge; MG: subject sharp; BG: bokeh warm lights". LUÔN 3 lớp để có chiều sâu.
- "atmos": 1 CHI TIẾT VẬT LÝ/KHÍ QUYỂN (English) suy được từ kịch bản — vd "steam curls from cup", "wind lifts curtain edge", "dust motes in sunbeam". BẮT BUỘC có để cảnh không chết đứng.
- "continuity": hint nối cảnh trước (English, để "" ở cảnh 1) — vd "matches previous scene's lighting direction and camera height".
...
{{"summary":"...","suggested_style":"cinematic","style_lock":"35mm grain, warm grade, soft key, shallow DOF","characters":[{{"name":"Mẹ","role":"chủ spa","age":"48","gender_presentation":"female","face":"round face, tired eyes","eyes":"dark brown","hair":"black shoulder-length tied back","skin_tone":"warm light","build":"height=158cm; build=average","wardrobe_top":"cream spa uniform tunic","wardrobe_bottom":"matching trousers","footwear":"white flats","headwear":"","accessories":"jade bracelet","distinguishing_marks":"laugh lines, small mole on right cheek","anchor":"jade bracelet + small mole on right cheek","palette":"cream, sage, gold","voice":"weary warm female","tts_voice":"Kore"}}],"scenes":[{{"beat":"Hook","chars":["CHAR_1"],"image":"...","action":"...","shot":"medium shot","lens":"35mm","camera_move":"static locked-off","lighting":"flat overcast daylight from a window camera-right, cool 5000K","mood":"defeated","audio":"quiet empty-room tone, faint ceiling-fan hum, distant street; sparse melancholic piano, low and unobtrusive","speaker":"CHAR_1","dialogue":"Cả ngày không có một mống khách nào hết...","prompt":"Medium shot, 35mm, static locked-off. Me slumps over an empty spa reception counter, head in hands, then looks up wearily. Quiet modern lobby, mid-afternoon. Flat overcast light from a window camera-right, muted shadows. Defeated, heavy mood; warm desaturated grade, shallow depth of field, subtle 35mm grain."}}]}}
<KICHBAN>
{script}
</KICHBAN>"""
...
(1) characters[] — HỒ SƠ NHÂN VẬT khoá để 1 người trông GIỐNG HỆT mọi cảnh. Suy từ nét vẽ + ghi chú; phần thiếu suy luận hợp lý & CỐ ĐỊNH. Các TRƯỜNG TÁCH RỜI (English): name, role, age, gender_presentation, face, eyes, hair, skin_tone (TRUNG TÍNH — không nhãn chủng tộc), build ("height=…cm; build=…"), wardrobe_top, wardrobe_bottom, footwear, headwear, accessories, distinguishing_marks (BẮT BUỘC), anchor (2 CHI TIẾT PHÂN BIỆT ghép bằng " + ": MỘT vật/phụ kiện độc + MỘT nét mặt/thân thể độc — vd "silver locket + scar above left eyebrow" — kết hợp 2 lớp lock chặt hơn, DẪN ĐẦU nhận dạng), palette, voice, tts_voice (Kore/Aoede/Leda=NỮ, Puck/Charon/Orus=NAM, khớp giới). KHÔNG gán id; liệt kê theo thứ tự XUẤT HIỆN.
...
- "beat" ({lang_label}), "chars" (list KHÓA/TÊN), "image" ({lang_label} — tả đúng những gì THẤY trong khung), "action" ({lang_label} — LIỆT KÊ 2-4 CỬ CHỈ/MICRO-MOTION suy được từ khung, vd "nghiêng đầu, ngón tay xoa cằm, liếc mắt xuống rồi ngước lên"; CẤM tính từ cảm xúc trần trụi không kèm động tác — mọi cảm xúc PHẢI hiển thị qua CỬ CHỈ CƠ THỂ).
- "shot","lens","camera_move","lighting","mood" (English; bám bố cục/góc máy SUY ĐƯỢC từ khung; ĐA DẠNG cú máy; LENS BẮT BUỘC focal length + aperture số cụ thể "50mm f/1.4"; ánh sáng nêu NGUỒN VẬT LÝ + nhiệt màu + THỜI ĐIỂM SỐ khi suy được).
- "depth": bố cục 3 lớp FG/MG/BG (English) — LUÔN 3 lớp để ra chiều sâu.
- "atmos": 1 CHI TIẾT VẬT LÝ/KHÍ QUYỂN suy được từ khung (English) — gió/khói/hạt bụi/ánh nến. BẮT BUỘC có.
- "continuity": hint nối cảnh trước (English, "" ở cảnh 1).
